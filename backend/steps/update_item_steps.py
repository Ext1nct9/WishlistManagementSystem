...
@given ('the system contains the following wishlists')
def step_impl(context):
...
@given ('the system contains the following user permissions')
def perm_impl(context):
    setup_initial_userPermission(DB_PATH)

@when('the valid user {validAccountId} request to update an item {validItemId} with name {validName}, description {description}, and status {status}')
def update_item_request(context, validAccountId, validItemId, validName, description, status):
    context.user_account_id = validAccountId
    context.description = description

    #Request
    context.response = requests.put(f'{BASE_URL}/update_item/{validItemId}', json={"user_account_id":validAccountId, "item_id":validItemId, "name" : validName, "description": description, "status": status}, cookies={"user_account_id":context.user_account_id})
    ...
    if context.response.status_code != 200:
        ...
        error_msg = f'Expected 200 status code for valid item update, but got {context.response.status_code}.'
        logging.error(error_msg)
        raise AssertionError(error_msg)
    assert context.response.status_code == 200

@when('the user {validAccountId} has permission {validPermission} to update wishlist {validWishlistId}')
...
def update_item_shall_exist(context, validItemId, validName, new_desc, validWishlistId):
    ...
    assert context.response.json()["item_id"] == expected_item["item_id"], "Response does not match the expected item ID"
    assert "name" in context.response.json(), "Failed to retrieve item name"
    assert context.response.json()["name"] == expected_item["name"], "Response does not match the expected item name"
    assert "description" in context.response.json(), "Failed to retrieve description"
    assert context.response.json()["description"] == expected_item["description"], "Response does not match the expected item description"
    assert "wishlist_id" in context.response.json(), "Failed to retrieve item"
    assert context.response.json()["wishlist_id"] == expected_item["wishlist_id"], "Response does not match the expected wishlist ID"