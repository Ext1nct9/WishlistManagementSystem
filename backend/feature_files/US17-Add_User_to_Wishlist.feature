Feature: Give an account permissions to a wishlist
  ...
  Scenario Outline: Successfully Add a User to a Wishlist
    ...
    When the user with id <owner_user_id> attempts to add a new UserPermission with permissions <permissions> to the Wishlist with id <owner_wishlist_id> for the user with email <other_email>
  ...
  Scenario Outline: Unsuccessfully Add a Non-Existent User to a Wishlist
    Given there is a UserAccount with id <owner_user_id> and username <owner_username> and email <owner_email> and password <owner_password>
    And there is a Wishlist with id <owner_wishlist_id> and name <owner_wishlist_name> and owner_id <owner_user_id>
    When the user with id <owner_user_id> attempts to add a new UserPermission with permissions <permissions> to the Wishlist with id <owner_wishlist_id> for the user with email <non_existent_user_email>
    Then the request returns the status code <status_code>
    ...
    And delete the Wishlist with id <owner_wishlist_id>
    Examples:
      | owner_user_id | owner_username | owner_email      | owner_password  | owner_wishlist_id | owner_wishlist_name | non_existent_user_email | permissions | status_code |
      | 1             | owner_name1    | owner1@email.com | owner_password1 | 1                 | wishlist_name1      | fake1@email.com         | 1           | 400         |
      | 3             | owner_name2    | owner2@email.com | owner_password2 | 2                 | wishlist_name2      | fake2@email.com         | 2           | 400         |
      | 5             | owner_name3    | owner3@email.com | owner_password3 | 3                 | wishlist_name3      | fake3@email.com         | 3           | 400         |
  ...
  Scenario Outline: Unsuccessfully Add a User to a Wishlist You Are Not the Owner Of
    ...
    And there is a Wishlist with id <owner_wishlist_id> and name <owner_wishlist_name> and owner_id <owner_user_id>
    And there is a UserAccount with id <other_user_id> and username <other_username> and email <other_email> and password <other_password>
    When the user with id <fake_owner_user_id> attempts to add a new UserPermission with permissions <permissions> to the Wishlist with id <owner_wishlist_id> for the user with email <owner_email>
    Then the request returns the status code <status_code>
    And the response contains an error message
